...
object I18NMessages {
  ...
  def format(messageId: String, lang: String, messageArguments: String*): String = {
    ...
    val s = messages.getOrElse(messageId,
      Locale.forLanguageTag(language), messageId)
    val template = new String(s.getBytes("ISO-8859-1"), "UTF-8")
    ...
    //    formatter.format(Array(messageArguments).mkString)
    formatter.format(Array(messageArguments)) // TODO prints WrappedArray(...
  }
}