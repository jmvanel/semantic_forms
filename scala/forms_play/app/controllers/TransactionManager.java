...
//        committerThread.start() ;
    ...
    public /*for testing only*/ static final boolean DEBUG = true; // jmv false ; 
    ...
    private DatasetGraphTxn begin$(ReadWrite mode, String label)
    {
        ...
        if ( DEBUG ) 
            switch ( mode )
            {
                case READ : System.out.println("JJ r") ; break ;
                case WRITE : System.out.println("JJ w") ; break ;
            }
        ...
        {  
            if ( DEBUG ) System.out.println("JJ commitedAwaitingFlush.size() " + commitedAwaitingFlush.size()) ;
            dsg = commitedAwaitingFlush.get(commitedAwaitingFlush.size()-1).getActiveDataset().getView() ;
        }
        else 
        {
            if ( DEBUG ) System.out.print("JJ _") ;
        }
    ...
    private void processDelayedReplayQueue(Transaction txn)
    {
        ...
        {
            if ( queue.size() > 0 ) 
                System.out.print("JJ "+ queue.size()+"!") ;
        }
    ...
    private void checkNodesDatJrnl(String label, Transaction txn)
    {
        if (txn != null)
        {
            String x = txn.getBaseDataset().getLocation().getPath(
                label + ": nodes.dat-jrnl") ;
            long len = new File(x).length() ;
            if (len != 0)
                log("JJ nodes.dat-jrnl: not empty", txn) ;
        }   
    }
    ...
    private final boolean logstate = true; // jmv (syslog.isDebugEnabled() || log.isDebugEnabled()) ;
