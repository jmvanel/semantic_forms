...
class MySimulation extends Simulation {
  ...
  val chain = exec( http("wordsearch" ).get("/wordsearch?q=bidon") )
      . exec( http("displayuri").get("/display?displayuri=http%3A%2F%2Fbblfish.net%2Fpeople%2Fhenry%2Fcard%23me&Display=Afficher") )
      . exec( http("create").get("/create?uri=&uri=http%3A%2F%2Fxmlns.com%2Ffoaf%2F0.1%2FPerson") )
      . exec( http("display jmvanel").get("/display?displayuri=http%3A%2F%2Fjmvanel.free.fr%2Fjmv.rdf%23me&Display=Afficher") )
