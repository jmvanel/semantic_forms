...
object SPARQLquery2SFcacheApp
  ...
  if( args . length < 3 ) {
    println("""Mirror SPARQL site into SF
cf https://github.com/jmvanel/semantic_forms/blob/master/doc/en/administration.md#mirror-sparql-site-into-sf-semantic-cache
The arguments are :
- SPARQL query string
- SPARQL endpoint
